...
export default class SatDefense {
  ...
  setControls() {
    ...
    setTimeout(() => {
      ...
      const handleButtonClick = (button, eventName, condition, action) => {
        const handleClick = (e) => {
          if (this.game.buttonActive) return;
          e.preventDefault();
          ...
          this.game.buttonActive = false; // Reset after each attempt
        };
  ...
  applySmallRepairs() {
    if (this.game.sats < 5) return false;
    this.game.sats -= 5;
    this.base.health = Math.min(21, this.base.health + 3);
    this.logEvent('Small repairs applied, +3 base health');
    return true;
  }
